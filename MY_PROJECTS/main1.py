...
class MainWindow(QMainWindow):
    ...
    def setup_toolbar(self):
        ...
        toolbar.setIconSize(QSize(42, 42))  # Увеличиваем размер иконок
    ...
    def show_module2(self):
        ...
        try:
            ...
            # Очищаем предыдущие док-виджеты расписания
            for dock in self.findChildren(QDockWidget):
                if dock.windowTitle().startswith("📅"):
                    print(f"🗑️ Удаляем старый док-виджет: {dock.windowTitle()}")
                    dock.deleteLater()
        
            # Создаем модуль расписания
            try:
                print("🔄 Создание ScheduleModule")
                self.schedule_module = ScheduleModule(self)
                print("✅ ScheduleModule создан")
            except Exception as e:
                print(f"❌ Ошибка создания ScheduleModule: {e}")
                QMessageBox.critical(self, "Ошибка инициализации", 
                                   f"Не удалось инициализировать модуль расписания:\n{str(e)}")
                return
        
            # Создаем док-виджет для модуля
            print("🔄 Создание док-виджета")
            dock = QDockWidget("📅 Расписание и Запись на прием", self)
            dock.setWidget(self.schedule_module)
            dock.setFeatures(QDockWidget.DockWidgetFeature.DockWidgetMovable | 
                            QDockWidget.DockWidgetFeature.DockWidgetFloatable)
        
            print("🔄 Добавление док-виджета в главное окно")
            self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dock)

            print("✅ Модуль расписания успешно показан")
        
        except Exception as e:
            print(f"💥 Критическая ошибка в show_module2: {e}")
            QMessageBox.critical(self, "Критическая ошибка", 
                               f"Произошла ошибка при открытии модуля:\n{str(e)}")

    def show_module3(self):
        self.status_bar.showMessage("Модуль: Электронная Медицинская Карта")
        QMessageBox.information(self, "ЭМК", "Модуль медицинских карт будет реализован")

    def show_module4(self):
        self.status_bar.showMessage("Модуль: Услуги и Складской учет")
        QMessageBox.information(self, "Склад", "Модуль склада будет реализован")

    def show_module5(self):
        self.status_bar.showMessage("Модуль: Финансы и Касса")
        QMessageBox.information(self, "Финансы", "Модуль финансов будет реализован")

    def show_module6(self):
        self.status_bar.showMessage("Модуль: Отчеты и Аналитика")
        QMessageBox.information(self, "Отчеты", "Модуль отчетов будет реализован")

    def show_module7(self):
        self.status_bar.showMessage("Модуль: Администрирование и Безопасность")
        QMessageBox.information(self, "Администрирование", "Модуль администрирования будет реализован")

    def show_module8(self):
        self.status_bar.showMessage("Модуль: Медицинское оборудование")
        QMessageBox.information(self, "Оборудование", "Модуль оборудования будет реализован")

    def show_module9(self):
        self.status_bar.showMessage("Модуль: ИИ-Ассистент")
        QMessageBox.information(self, "ИИ-Ассистент", "Модуль ИИ-ассистента будет реализован")

    def show_module10(self):
        self.status_bar.showMessage("Модуль: Техподдержка")
        QMessageBox.information(self, "Техподдержка", "Модуль техподдержки будет реализован")
