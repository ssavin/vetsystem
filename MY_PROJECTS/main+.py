...
class MainWindow(QMainWindow):
    ...
    def show_module2(self):
        ...
        dock.setFeatures(QDockWidget.DockFeature.DockWidgetMovable | 
                        QDockWidget.DockFeature.DockWidgetFloatable)
    
        self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dock)        
        # Создаем или показываем модуль расписания
        if not hasattr(self, "schedule_module") or self.schedule_module is None:
            self.schedule_module = ScheduleModule(self)
            
        # Создаем док-виджет для модуля
        dock = QDockWidget("📅 Расписание приемов", self)
        dock.setWidget(self.schedule_module)
        dock.setFeatures(QDockWidget.DockFeature.DockWidgetMovable | 
                        QDockWidget.DockFeature.DockWidgetFloatable)
