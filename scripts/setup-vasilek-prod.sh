...
# Увеличиваем лимит памяти Node.js до 4GB — для больших объёмов данных
...
echo "📋 Шаг 9: Миграция осмотров (~456,000 записей, ~90-120 мин)..."
echo "   ВНИМАНИЕ: самый долгий шаг — каждая запись содержит тексты осмотров"
DATABASE_URL="$PROD_DB" $TSX scripts/migrate-medical-data.ts \
  --tenant bd89523e-47e7-4d4b-8b94-e98c6d3e1959 \
  --db vetais_vasilek \
  --host 94.198.53.52 \
...
# ── Шаг 11: Счета ─────────────────────────────────────────────────────────
echo "🧾 Шаг 11: Миграция счетов (~472,000 счётов + 1.6M позиций, ~60-90 мин)..."
DATABASE_URL="$PROD_DB" $TSX scripts/migrate-medical-data.ts \
  --tenant bd89523e-47e7-4d4b-8b94-e98c6d3e1959 \
  --db vetais_vasilek \
  --host 94.198.53.52 \
