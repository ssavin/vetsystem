...
export class DocumentService {
  ...
  async buildLabResultsHtml(orderId: string): Promise<string> {
    const order = await storage.getLabOrder(orderId);
    ...
    const URGENCY_LABELS: Record<string, string> = {
      routine: 'Плановый', urgent: 'Срочный', stat: 'Немедленно',
    };
    ...
    const results = resultDetails.map((r: any) => {
      ...
      return {
        ...
        value: r.numericValue != null ? Number(r.numericValue).toString() : (r.value ?? '—'),
        unit: param?.unit ?? '',
        status: statusLabel,
        rowClass: isCritical ? 'critical' : isAbnormal ? 'abnormal' : '',
        statusClass: isCritical ? 'critical' : isAbnormal ? '' : 'normal',
        notes: r.notes ?? '',
      };
    });
    ...
    const resultTable = results.length > 0 ? `
      <table>
        <thead>
          <tr><th>Параметр</th><th>Значение</th><th>Ед. изм.</th><th>Статус</th><th>Примечания</th></tr>
        </thead>
        <tbody>${resultRows}</tbody>
      </table>` : '<p><em>Результаты не введены</em></p>';
...
<div class="info-grid">
  ...
  <div class="info-item"><div class="info-label">Вид / Порода</div><div class="info-value">${patient?.species ?? '—'}${patient?.breed ? ` (${patient.breed})` : ''}</div></div>
  <div class="info-item"><div class="info-label">Владелец</div><div class="info-value">${ownerName}</div></div>
  <div class="info-item"><div class="info-label">Врач</div><div class="info-value">${(doctor as any)?.name ?? '—'}</div></div>
...
<div class="footer">Распечатано: ${new Date().toLocaleDateString('ru-RU')} ${new Date().toLocaleTimeString('ru-RU')} &nbsp;|&nbsp; Документ носит информационный характер.</div>
</body>
</html>`;
  }
}
